  ...
  var div;
	...
	function randomnum()
	{
	 required=10+Math.floor(Math.random()*10);
	 if(required==10)
	 {
		 required=19;
	 }
 ...
 */
	    ...
	    initialiseOtherVariables();
	  	...
	  	material = new THREE.MeshBasicMaterial( {color: 0x88ade8} );
	  		...
	  		arrow.rotateZ(-Math.PI/2);
				...
				bundle[2].position.set(centerX+30,centerY-19,0);
				...
				sticks[38].position.set(centerX+30,centerY-15,0);
	...
	function move(element)
	{
		if(tempRequired >= 0)
		{
			...
			{
				cText[required- tempRequired +4].visible=true;
				sticks[required- tempRequired +18].visible=true;
				tempRequired--;
				collected++;
				stickCollected++;
				tempElement = sticks[tempRequired];
	...
	function updateExperimentElements(t, dt)
	{
		...
		if(sticks[i].position.x > centerX-15 && sticks[i].position.y> centerY+5 && sticks[i].position.x < centerX)
		{
			...
			if(stickAdd[i]==0)
			{
				sticks[stickCollected+19].visible=true;
	...
	if(success==1)
	{
		...
		if(totStick.innerHTML=="10" && totBundle.innerHTML=="0")
		{
			stickCollected -= 10;
			totStick.innerHTML=stickCollected;
			bundleCollected++;
	...
	}if(collected==10 && bundleCollected==0)
	{
		for (var i = 21; i < 31; i++) {
			sticks[i].visible=true;
			if(i<26)
		...
		if(sticks[30].position.x<centerX+31)
		{
			bundle[3].position.set(centerX+30,centerY+17,0);
			cText[24].position.set(centerX+30,centerY+14,0)
		}
}
	/******************* Update (animation changes) code ***********************/
