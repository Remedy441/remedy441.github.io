...
function initialiseHelp(){
    helpContent="";
    helpContent = helpContent + "<h2> Current through wires of different length.</h2>";
    ...
    helpContent = helpContent + "<p>The initial state is setup stage. In this stage select any wire of any length provided.</p>";
...
function initialiseInfo(){
    infoContent =  "";
    infoContent = infoContent + "<h2> Current through wires of different length concepts</h2>";
    infoContent = infoContent + "<h3>About the experiment</h3>";
    infoContent = infoContent + "<p>The experiment shows the variation of current with the variation in length of a wire with constant radius and resistivity.</p>";
    infoContent = infoContent + "<h3>Current Flow</h3>";
    infoContent = infoContent + "<p>Resistance is directly proportional to length. </p>"
    infoContent = infoContent + " Hence conductance is inversly proportional to length.</p>" 
    infoContent = infoContent + "<p>Here we have a fixed voltage and varying radius of bulb.</p>";
    infoContent = infoContent + "<p>Here current in directly proportional to conductance, so when conductance increases(length decreases), the current increases.</p>";
        ...
        if(intersects[0].object == Switch || intersects[0].object == fakeSwitch){
            ...
            for(var i=0;i<6;i++){
                if(intersects[0].object == lamp[i]){
                    ...
                    clamp=i;
                    btmesh[i].position.set(-2+ox-1.5,2.5,0.41);
...
function createLamp(n,r){
    lamp[n] = new THREE.CylinderGeometry(0.2, 0.2, 1*r, 32);
...
function declareElements(){
    ...
    //lamp starts
    str[0]="8.00 A";str[1]="5.33 A";
    str[2]="4.00 A";str[3]="3.20 A";
    str[4]="2.67 A";str[5]="2.28 A";
    str[6]="2.00 A";
    r[0]=1;r[1]=1.5;r[2]=2;r[3]=2.5;r[4]=3;r[5]=3.5;r[6]=4;
    for(var i=0;i<6;i++){
        createLamp(i,r[i]);
    }
    //lamp ends
...
function addbText(){
        ...
        loader.load( 'optimer_regular.typeface.json', function ( font ) {
            ...
            var textGeometry2 = new THREE.TextGeometry( (10)*r[i]+'mm', {
                font: font,
                size: 0.5,
                height:0.1,
                curveSegments: 12,
...
function positionElements(){
    ...
    addWire(-4.95+ox,3.93,2.94,Math.PI/2); //Wire4
    addWire(1.95+ox,3.93,8.94,Math.PI/2);  //Wire5
    addWire(-4+ox,-5,4.84,Math.PI/2);          //Wire6
    addWire(4+ox,-5,4.84,-Math.PI/2);          //Wire7
    bstr[0]="10 mm";bstr[1]="15 mm";
    bstr[2]="20 mm";bstr[3]="25 mm";
    bstr[4]="30 mm";bstr[5]="35 mm";
...
function loadExperimentElements(){
    st=0;dir=0;
    PIEsetExperimentTitle("Current through wires of different length");
...
function updateExperimentElements(t, dt){
    rectMove(6.5+ox,-6.5+ox,3.93+oy,-5+oy,0.0067*(6-clamp)/2.5,dt,charge);
    PIErender();
}