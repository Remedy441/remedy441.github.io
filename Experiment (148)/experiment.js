//cars, their distances, and distancepanels
var cars=[];
var dist=[], distpanel=[];
var correct, wrong;
var timeoutcalls=[];
...
todo : write seconds on timeteller - done
...
function loadExperimentElements(){
    ...
    tex=loader.load("road5.png", function(){
        PIErender();
    });
    ...
    //finishline
    tex=loader.load("checkerboard.jpg", function(){
        PIErender(); 
    });
    ...
    tex = loader.load( '../../../PIE/images/grasslight-big.jpg', function(){
        PIErender(); 
    });
    tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
    ...
    for(var cnt=1; cnt<=4; cnt++)
        loader.load("car"+cnt+".json", whysoasynchronous(cnt));
    
    //the big parallelopiped on top (i hope its called parallelopiped)
    loader.load("timeteller.json", function(geometry, materials) {
        ...
        timeteller.translation = geometry.center();
        timeteller.castShadow=timeteller.receiveShadow=true;
        timeteller.position.set(0, 6, -2);
        
        PIEaddElement(timeteller);
        PIErender();
    });
    ...
    loader.load( "optimer.json", function(response){
        ...
        for(var i=1; i<=4; i++){
            geometry = new THREE.TextGeometry("T I M E", {
                font : font,
                size : 0.3,
                height : 0.25,
                curveSegments : 3
            ...
            geometry = new THREE.TextGeometry("seconds", {
                font : font,
                size : 0.225,
                height : 0.25,
                curveSegments : 3
...
//position and rotation of "T I M E" and "seconds" texts on the clock
...
function blink(color){
    timeteller.material.materials[0].color.setHex(color);
...
function whysoasynchronous(cnt){
    return function(geometry, materials) {
        ...
        distpanel[cnt] = new THREE.Mesh(geo, mate);
        distpanel[cnt].position.set(cars[cnt].position.x-0.4, 0.85, cars[cnt].position.z);
        distpanel[cnt].lookAt(PIEcamera.position);
        distpanel[cnt].material.transparent=true;
        distpanel[cnt].material.opacity=0.7;
        distpanel[cnt].material.emissive.setHex(hex);

        PIEaddElement(cars[cnt]);
        PIEaddElement(distpanel[cnt]);
        distpanel[cnt].receiveShadow=false;
        PIErender();
    }
}
function updateExperimentElements(t, dt){
    ...
    for(var i = 1; i<=4; i++){
        ...
        distpanel[i].position.x=cars[i].position.x-0.4;
        distpanel[i].lookAt(PIEcamera.position);

        //update the 3d text
        PIEscene.remove(thetime[i]);
        PIEscene.remove(dist[i]);
        settime(i, displayTime.toPrecision(displayTime<10 ? 3:4));
        setdist(i, (cars[i].position.x+32).toPrecision(4));
    }
...
function setdist(i, text){
    distgeo = new THREE.TextGeometry(text+' m', {
        font : font,
        size : 0.15,
        height : 0.05,
        curveSegments : 3
    ...
    dist[i]=new THREE.Mesh(distgeo, matfront);
    dist[i].translation = distgeo.center();
    dist[i].position.set(cars[i].position.x-0.35, 0.85, cars[i].position.z);
    dist[i].lookAt(PIEcamera.position);
    PIEaddElement(dist[i]);
    dist[i].castShadow=false;
}

//changes/adds the time on the parallelopiped
function settime(index, text){
    ...
    thetime[index]=new THREE.Mesh(geometry, fontmat);
    thetime[index].translation = geometry.center();

    PIEaddElement(thetime[index]);  
    thetime[index].castShadow=false;
}

function settimeprops(){
    thetime[1].position.set(0, 6, -0.8)
    thetime[1].rotation.set(0.29, 0, 0);

    thetime[2].position.set(1.2, 6, -2)
    thetime[2].rotation.set(Math.PI/2, Math.PI/2 - 0.27, 3*Math.PI/2);

    thetime[3].position.set(0, 6, -3.2);
    thetime[3].rotation.set(-0.29, Math.PI,0);

    thetime[4].position.set(-1.2 ,6 , -2)
    thetime[4].rotation.set(Math.PI/2, -Math.PI/2 + 0.27, Math.PI/2);    
}
...
function initialiseOtherVariables(){
    ...
    for(var cnt = 1; cnt<=4; cnt++){
        ...
        PIEscene.remove(thetime[cnt]);
        settime(cnt, 0.00);
    }             

    dist[1].position.set(-32, 0.85, -7);
    dist[2].position.set(-32, 0.85, -4);
    dist[3].position.set(-32, 0.85, 0);
    dist[4].position.set(-32, 0.85, 3);

    distpanel[1].position.set(-32-0.4, 0.85, -7);
    distpanel[2].position.set(-32-0.4, 0.85, -4);
    distpanel[3].position.set(-32-0.4, 0.85, 0);
    distpanel[4].position.set(-32-0.4, 0.85, 3);
...
function initialiseHelp()
{
    ...
    helpContent = helpContent + "</ul>";
    helpContent = helpContent + "<p>All four cars have their respective distance traversed shown above them. Time taken is, obviously, same for all four.</p>";
