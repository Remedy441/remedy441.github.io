...
var myRIncidentRay1;
var myRIncidentRay1XStart;
var myRIncidentRay1XEnd;
var myRIncidentRay1YStart;
var myRIncidentRay1YEnd;
var myRIncidentRay1ZStart;
var myRIncidentRay1ZEnd;

var myRIncidentRay2;
var myRIncidentRay2XStart;
var myRIncidentRay2XEnd;
var myRIncidentRay2YStart;
var myRIncidentRay2YEnd;
var myRIncidentRay2ZStart;
var myRIncidentRay2ZEnd;
...
function handleRad(newValue) {
    ...
    myConvexMirrorRadius = newValue;

    myConvexMirror.geometry = new THREE.SphereGeometry(myConvexMirrorRadius, segmentWidth, segmentHeight, phiStart + 0.4, phiLength, thetaStart, thetaLength);

    myConvexMirror.geometry.verticesNeedUpdate = true;
...
function initialiseControlVariables()
{
    ...
    defaultRad=myConvexMirrorRadius;
...
function initializeStaticVariables()
{
    ...
    /* Initialise Convex Mirror variables */
    ...
    thetaStart      =   3.14;
    thetaLength     =   3.14;
    myConvexMirrorRadius = mySceneW/15;

    myConvexMirrorX        = 1.4*myCenterX;
    myConvexMirrorY        = myAllY;
    myConvexMirrorZ        = myAllZ;
...
function initalizeDynamicVariables()
{
    ...
    myObjectDragBallDist    =   myConvexMirrorX-myObjectDragBallX-myConvexMirrorRadius;
    ...
    ballBLeft       =bLeft;
    ballBRight      =myConvexMirrorX-myConvexMirrorRadius-2*myObjectDragBallRadius;

    /*Focal Points Initialize*/

    myBallFRadius   =   mySceneW/250;
    myBallFX        =   myConvexMirrorX-myConvexMirrorRadius/2;
    myBallFY        =   myAllY;
    myBallFZ        =   myAllZ;

    myBall2FRadius  =   mySceneW/250;
    myBall2FX       =   myConvexMirrorX;
    myBall2FY       =   myAllY;
    myBall2FZ       =   myAllZ;
...
function initializeRayVariables()
{
    ...
    myIncidentRay1XDiff     = myConvexMirrorRadius-Math.sqrt(myConvexMirrorRadius*myConvexMirrorRadius-myObjectLen*myObjectLen);

    myIncidentRay1XStart    = myObjectXEnd;
    myIncidentRay1XEnd      = myConvexMirrorX-myConvexMirrorRadius+myIncidentRay1XDiff;
    ...
    myIncidentRay2Slope     = (myBall2FY-myObjectYEnd)/(myBall2FX-myObjectXEnd);
    a=myBall2FX;
    b=myBall2FY;
    m=myIncidentRay2Slope;
    r=-1*myConvexMirrorRadius;

    c=r/Math.sqrt(m*m+1)+a;
    d=(m*r)/Math.sqrt(m*m+1)+b;

    myIncidentRay2XStart    = myObjectXEnd;
    myIncidentRay2XEnd      = c;
    myIncidentRay2YStart    = myObjectYEnd;
    myIncidentRay2YEnd      = d;
    myIncidentRay2ZStart    = myObjectZEnd;
    myIncidentRay2ZEnd      = myObjectZEnd;


    myIncidentRay3XStart    = myObjectXEnd;
    myIncidentRay3XEnd      = myConvexMirrorX-myConvexMirrorRadius;
    myIncidentRay3YStart    = myObjectYEnd;
    myIncidentRay3YEnd      = myConvexMirrorY;
    myIncidentRay3ZStart    = myObjectZEnd;
    myIncidentRay3ZEnd      = myObjectZEnd;


    myRIncidentRay1XStart   = myIncidentRay1XEnd;
    myRIncidentRay1XEnd     = myBallFX;
    myRIncidentRay1YStart   = myIncidentRay1YEnd;
    myRIncidentRay1YEnd     = myBallFY;
    myRIncidentRay1ZStart    = myObjectZEnd;
    myRIncidentRay1ZEnd      = myObjectZEnd;


    myRIncidentRay2XStart   = myIncidentRay2XEnd;
    myRIncidentRay2XEnd     = myBall2FX;
    myRIncidentRay2YStart   = myIncidentRay2YEnd;
    myRIncidentRay2YEnd     = myBall2FY;
    myRIncidentRay2ZStart    = myObjectZEnd;
    myRIncidentRay2ZEnd      = myObjectZEnd;
    ...
    myReflectedRay1XEnd     = myCenterX-2;
...
function updateRayVariables()
{
    ...
    lineGeometry = new THREE.Geometry();
    vertArray = lineGeometry.vertices;
    vertArray.push( new THREE.Vector3(myRIncidentRay1XStart, myRIncidentRay1YStart, myRIncidentRay1ZStart), new THREE.Vector3(myRIncidentRay1XEnd, myRIncidentRay1YEnd, myRIncidentRay1ZEnd) );
    lineGeometry.computeLineDistances();
    myRIncidentRay1.geometry=lineGeometry;

    myRIncidentRay1.verticesNeedUpdate=true;

    lineGeometry = new THREE.Geometry();
    vertArray = lineGeometry.vertices;
    vertArray.push( new THREE.Vector3(myRIncidentRay2XStart, myRIncidentRay2YStart, myRIncidentRay2ZStart), new THREE.Vector3(myRIncidentRay2XEnd, myRIncidentRay2YEnd, myRIncidentRay2ZEnd) );
    lineGeometry.computeLineDistances();
    myRIncidentRay2.geometry=lineGeometry;

    myRIncidentRay2.verticesNeedUpdate=true;
...
function initialiseReflectedArrow() {
    ...
    myReflectedRay1MidpointX     =   ((myReflectedRay1XEnd+myReflectedRay1XStart)/2+myReflectedRay1XStart)/2;
    myReflectedRay1MidpointY     =   ((myReflectedRay1YEnd+myReflectedRay1YStart)/2+myReflectedRay1YStart)/2;
...
function loadExperimentElements()
{
    ...
    /* Create Convex Mirror and add it to scene */
    geometry=new THREE.SphereGeometry(myConvexMirrorRadius,segmentWidth,segmentHeight,phiStart+0.4, phiLength, thetaStart, thetaLength);
    material=new THREE.MeshBasicMaterial({color: 0x3F51B5});
    myConvexMirror = new THREE.Mesh(geometry,material);
    myConvexMirror.position.set(myConvexMirrorX, myConvexMirrorY, myConvexMirrorZ);
    PIEaddElement(myConvexMirror);
    ...
    vertArray.push( new THREE.Vector3(myRIncidentRay1XStart, myRIncidentRay1YStart, myRIncidentRay1ZStart), new THREE.Vector3(myRIncidentRay1XEnd, myRIncidentRay1YEnd, myRIncidentRay1ZEnd) );
    lineGeometry.computeLineDistances();
    lineMaterial = new THREE.LineBasicMaterial( { color: 0x3F51B5,linewidth: 3 } );
    ...
    myRIncidentRay1 = new THREE.Line( lineGeometry, lineMaterial );

    PIEaddElement(myRIncidentRay1);
    ...
    lineGeometry.computeLineDistances();
    lineMaterial = new THREE.LineBasicMaterial( { color: 0x3F51B5,linewidth: 3 } );
    myRIncidentRay2 = new THREE.Line( lineGeometry, lineMaterial );

    PIEaddElement(myRIncidentRay2);
...
function updateExperimentElements(t, dt)
{
    if(con<=100) {
        ...
        if(con<=150)
        {
            x4=myRIncidentRay1XStart;
            x5=myRIncidentRay1XEnd;
            y4=myRIncidentRay1YStart;
            y5=myRIncidentRay1YEnd;

            len=Math.sqrt((x5-x4)*(x5-x4)+(y5-y4)*(y5-y4));
            m=(y5-y4)/(x5-x4);
            dl=len/50;
            k=con-100;
            x6=x1-(k*dl)/(Math.sqrt(m*m+1));
            y6=m*(x6-x4)+y1;
            lineGeometry = new THREE.Geometry();
            vertArray = lineGeometry.vertices;
            vertArray.push( new THREE.Vector3(x4, y4, myAllZ), new THREE.Vector3(x5, y5, myAllZ));
            lineGeometry.computeLineDistances();

            myRIncidentRay1.geometry=lineGeometry;
            myRIncidentRay1.verticesNeedUpdate=true;

            myRIncidentRay1.visible=true;
    ...
    else if(con<=400)
    {
        ...
        if(con<=350)
        {
            x4=myRIncidentRay2XStart;
            x5=myRIncidentRay2XEnd;
            y4=myRIncidentRay2YStart;
            y5=myRIncidentRay2YEnd;
...
{
    helpContent="";
    helpContent = helpContent + "<h2>Tracing rays at a Convex mirror</h2>";
    helpContent = helpContent + "<h3>About the experiment</h3>";
    helpContent = helpContent + "<p>This experiment focuses on Tracing rays at a Convex mirror</p>";
    ...
    helpContent = helpContent + "<p>User can also change the radius of curvature of Convex mirror</p>";
    ...
    helpContent = helpContent + "<p>On pressing the start button you will be able to see each ray being drawn and also able to see how these rays are reflected from the Convex mirror following the Snell's Law.</p>";
...
{
    infoContent =  "";
    infoContent = infoContent + "<h2>Tracing rays at a Convex mirror</h2>";
    infoContent = infoContent + "<h3>About the experiment</h3>";
    infoContent = infoContent + "<p>This experiment focuses on Tracing rays at a Convex mirror</p>";
    infoContent = infoContent + "<h3>Image formation by a Convex Mirror</h3>";
    infoContent = infoContent + "<p>Image is formed in a Convex mirror when the reflected rays appear to meet in front of the mirror .This point of intersection gives us the idea about formaton of the image." +
                                ...
                                "Image formed by Convex mirror is always imaginary, size of image is smaller than the object.</p>";
