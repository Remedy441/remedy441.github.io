...
function myBar1Drag(element, newpos)
{
	...
	bar1y = newpos.y;
	bar1x = newpos.x;
	if(bar1y != 1.455556035041809)
	{
		bar1y = 1.455556035041809;
	}
	if(bar1x >= -3)
	{
		bar1x = -3;
	}
	bar1.position.set(bar1x,bar1y,newpos.z);
}


// function handlePosition_x(newValue)
// {
// 	// myBallX = newValue;
// 	originalx=newValue;
// 	original.position.x = originalx;
// 	 original.position.set(originalx, 0,0);
...
function initialiseControls()
{
   // initialiseControlVariables();
    ...
    PIEaddInputSlider("Focal length",15, handleFocal_length,10,19, 1);

    /* Create Display Panel */
    PIEaddDisplayText("Object Position(x)",-15);
...
function initialiseHelp()
{
    ...
    helpContent = helpContent + "<p>The top line has animation controls. There are two stages of the experiment.</p>";
    helpContent = helpContent + "<h3>The setup stage</h3>";
    helpContent = helpContent + "<p>The initial stage is setup stage. In this stage, you can see a control window at the right. You have access to one slider.</p>";
    helpContent = helpContent + "<p>You can control the following:</p>";
    helpContent = helpContent + "<ul>";
    helpContent = helpContent + "<li>Focal length&nbsp;&nbsp;:&nbsp;Controls the focal length of the lens.</li>";
     helpContent = helpContent + "<p>You can setup the initial position of object by dragging it along the princial axis to the desired position.</p>";
    helpContent = helpContent + "</ul>";
    ...
    helpContent = helpContent + "<ul>";
    helpContent = helpContent + "<li>Object Position(x)&nbsp;&nbsp;:&nbsp;Shows the X coordinate of the object on the principal axis.</li>";
    helpContent = helpContent + "<li>Focal length&nbsp;&nbsp;:&nbsp;Shows the focal length of the lens.</li>";
    helpContent = helpContent + "</ul>";
        helpContent = helpContent + "<h5> While the animation is on, you can change the position of object by dragging and see the corresponding image formed by the lens.</h5>";
...
var texture;
   ...
   // geometry1=new THREE.BoxGeometry(mySceneW *2,mySceneH *2,0);
       ...
       geometry=new THREE.PlaneGeometry( mySceneW*2 , mySceneH  );
   ...
   material = new THREE.MeshLambertMaterial( {color: 0xaaaaaa} );
    ...
    myLeft.position.set(2*leftB, myCenterY, 0.0);
	...
	flame2=loader.parse(flame1);
	flame2.position.x=0;
	flame2.position.y=1.5;
	PIEaddElement(flame2);

	bar1=loader.parse(bar);
	PIEaddElement(bar1);
	bar1.add(flame2);
	PIEdragElement(bar1);
	PIEsetDrag(bar1, myBar1Drag);
	bar1.position.x=-30;
	...
	// pastoriginalx = -5;
     ...
     PIEcreateTable("Size and Orientation of Image at various points:", 6, 4, true);
      var headerRow=["Position Of Object", "Position Of Image  ", "Relative Size Of Image  ", "Nature Of Image  "];
       var Row1=["At infinity ", "&nbsp;At focus F2", "&nbsp;Highly diminished", "&nbsp;Real and inverted"];
       var Row2=["Beyond 2F1", "&nbsp;Between F2 & 2F2", "&nbsp;Diminished", "&nbsp;Real and inverted"];
       var Row3=["At 2F1 ", "&nbsp;At 2F2", "&nbsp;Same sized", "&nbsp;Real and inverted"];
       var Row4=["Between F1 and 2F1 ", "&nbsp;Beyond 2F2", "&nbsp;Enlarged", "&nbsp;Real and inverted"];
       var Row5=["At focus F1 ", " &nbsp;At infinity", "&nbsp;Infinitely large", "&nbsp;Real and inverted"];
       var Row6=["Between F1 and O ", "&nbsp;Same side of object", "&nbsp;Enlarged", "Virtual and Magnified"];
      ...
      PIEupdateTableRow(5,Row5);
      PIEupdateTableRow(5,Row6);


	originalx = bar1.position.x;
	pt1x = originalx;
	pt1y = flame2.position.y +2;
	pt2y = flame2.position.y +2;

	 pt1 = new THREE.Vector3(originalx,flame2.position.y+2,0);
	 og = new THREE.Vector3(0,0,0);
	 pt2 = new THREE.Vector3(0,flame2.position.y+2,0);
...
function resetExperiment() {
  ...
  cleanup();
		...
		img2.visible =false;
		 ...
		 flag3=0;
		 flag4=0;
		 bar1.position.x = - 2*focal_length;
...
var time = dt/1000;
    ...
    if ( pastoriginalx != bar1.position.x )
    {
    	// PIEstopAnimation();
		PIEscene.remove(arrowHelper);
		PIEscene.remove(arrowHelper1);
	...
	focus = new THREE.Vector3(focal_length,0,0);
  	...
  	focal_length = PIEgetInputSlider("Focal length"); // focal length or position of focus
	...
	focus = new THREE.Vector3(focal_length,0,0);
		...
		if(increment >= (0-pt1x)){ flag1=1;  }  //PIEaddElement(arrowHelper);}


			////left down geometry 2 increment1 d h1 flag2
		var d = Math.sqrt(pt1x*pt1x + pt1y*pt1y) ;
			if(increment1 < (Math.sqrt(pt1x*pt1x + pt1y*pt1y)))
			{
	...
	if(slope1!=slope2)
	{
		...
		////Right down...geometry 4 increment2 d2 h2 flag3 depends on left down so flag 2
			...
			var d2 = Math.sqrt(intersectionx*intersectionx + intersectiony*intersectiony) ;
			if((increment2 <= d2)  && (flag2==1))
			{
			...
			if(increment2 >= d2){flag3=1; } //PIEaddElement(arrowHelper2);}



		//////////right up geometry 3 increment3 d3 h3 flag4 depends on left up so flag 1

			if((increment3 < d3) && flag1 )  //
			{
			...
			if(increment3 >= d3){flag4=1;  } //PIEaddElement(arrowHelper3);}
			...
			if(intersectionx > 0)
			{
				if(flag3 && flag4)
				{
					...
					img2.scale.y = 1.2 * Math.abs(intersectionx)/Math.abs(originalx);
					img2.visible = true;
				}
			}
			else
	...
	{
		 intersectionx = 500;
		intersectiony = intersectionx*slope1;
		 intersection=new THREE.Vector3(intersectionx,intersectiony,0);

		intersectionx=500;
		intersectiony=slope2*intersectionx+pt2.y;
		...
		//////////right up geometry 3 increment3 d3 h3 flag4 depends on left up so flag 1
			...
			//clock.innerHTML+="<i> :Characteristics of IMAGE: </i></u><b> Real,at Infinity</b>";

		// document.getElementById('description').innerHTML="Characteristics of IMAGE:Real,at Infinity";


	}


 	PIEchangeDisplayText("Object Position(x)",originalx);
