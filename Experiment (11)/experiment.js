...
function setOn(){
	...
	oscCount = 0;
	//controlball.rotation.z = -Math.PI/2.3;
...
//	PIEaddElement(pendulum);
	...
	controlball.remove(pendulum);
	pendulum = new THREE.Mesh(new THREE.SphereGeometry(1,32,24),new THREE.MeshPhongMaterial({color:"red"}));
	pendulum.position.set(x,-wirelength/2+11,0);
	PIEaddElement(pendulum);
	PIEdragElement(pendulum);
    PIEsetDrag(pendulum, mypendulumDrag);
    */
...
//	controlball.remove(wire);
//	wire = new THREE.Mesh(new THREE.CylinderGeometry(0.05,0.05,wirelength,32),new THREE.MeshBasicMaterial({color:"black"}));
//	controlball.add(wire);
//	wire.position.set(0,-wirelength/2,0);

//	pendulum = new THREE.Mesh(new THREE.SphereGeometry(1,32,24),new THREE.MeshBasicMaterial({color:"white"}));
//	controlball.add(pendulum);
//	pendulum.position.set(0,-wirelength,0);
	
	PIEchangeDisplayText(osc,0);
	PIEchangeDisplayText(timeString,0);
	
//	controlball.rotation.z = -Math.PI/2.31;
...
/*	geometry.vertices.push(new THREE.Vector3(0, 11, 0));
	geometry.vertices.push(new THREE.Vector3(mypenX,mypenY,mypenZ));
...
function loadExperimentElements() {
	...
	//controlball.rotation.z = -Math.PI/2.31;
	 //wire = new THREE.Mesh(new THREE.CylinderGeometry(0.05,0.05,11,32),new THREE.MeshBasicMaterial({color:"black"}));
// 	PIEaddElement(wire);
	//controlball.add(wire);
	//wire.position.set(0,-5.5,0);
	//wire.visible = false;
    ...
    myButton();
    //PIEaddButton("submit");
   // var resetButton  = PIEaddButton("Reset");
   // resetButton.style.height = "50%";
    //resetButton.style.width="50%";
  // resetButton.innerHTML = "<i class=\"fa fa-undo\" aria-hidden=\"true\"></i>";
...
{  
	...
	//rotateAroundWorldAxis(pendulum,new THREE.Vector3(0,11,0),0.5);
	/*
	else if(controlball.rotation.z >= Math.PI/2.3 && direction>0 ){
		console.log("fattu");
		direction = -1; 
		controlball.rotation.z = Math.PI/2.3;
	}
	if(controlball.rotation.z > -Math.PI/2.3 && (direction<0) ){
		controlball.rotation.z = controlball.rotation.z - Math.pow(wirelength,-1)*Math.cos(Math.abs(controlball.rotation.z))*Math.cos(Math.abs(controlball.rotation.z));
	}
	else if(controlball.rotation.z <= - Math.PI/2.3 && direction<0 ){
		direction = 1; 
		controlball.rotation.z = -Math.PI/2.3;
